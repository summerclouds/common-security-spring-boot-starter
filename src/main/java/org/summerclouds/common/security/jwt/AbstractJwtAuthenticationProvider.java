...
public abstract class AbstractJwtAuthenticationProvider implements AuthenticationProvider, InitializingBean, MessageSourceAware {
	...
	protected Authentication createSuccessAuthentication(Object principal, Authentication authentication,
		...
		JwtAuthenticationToken result = new JwtAuthenticationToken(jwtAuth.getToken(), jwtAuth.getClaims(),
	 ...
	 */
	protected abstract UserDetails retrieveUser(String username, JwtAuthenticationToken authentication)
	...
	private class DefaultPreAuthenticationChecks implements UserDetailsChecker {
		...
		public void check(UserDetails user) {
			if (!user.isAccountNonLocked()) {
				AbstractJwtAuthenticationProvider.this.logger
						.debug("Failed to authenticate since user account is locked");
				throw new LockedException(AbstractJwtAuthenticationProvider.this.messages
						.getMessage("AbstractUserDetailsAuthenticationProvider.locked", "User account is locked"));
			}
			if (!user.isEnabled()) {
				AbstractJwtAuthenticationProvider.this.logger
						.debug("Failed to authenticate since user account is disabled");
				throw new DisabledException(AbstractJwtAuthenticationProvider.this.messages
						.getMessage("AbstractUserDetailsAuthenticationProvider.disabled", "User is disabled"));
			}
			if (!user.isAccountNonExpired()) {
				AbstractJwtAuthenticationProvider.this.logger
						.debug("Failed to authenticate since user account has expired");
				throw new AccountExpiredException(AbstractJwtAuthenticationProvider.this.messages
	...
	private class DefaultPostAuthenticationChecks implements UserDetailsChecker {
		...
		public void check(UserDetails user) {
			if (!user.isCredentialsNonExpired()) {
				...
				throw new CredentialsExpiredException(AbstractJwtAuthenticationProvider.this.messages
