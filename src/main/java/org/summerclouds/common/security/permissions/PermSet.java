...
public class PermSet implements Iterable<Perm>, Permissions {
    ...
    public PermSet(String... permissions) {
        for (String perm : permissions)
            for (String perm2 : perm.split(PERMISSION_DIVIDER))
                if (MString.isSetTrim(perm2)) add(new Perm(perm2));
    }
    ...
    protected void add(Perm perm) {
        ...
        if (!permissions.add(perm)) return; // already added - maybe with different comment
        ...
        for (String action : perm.getActions()) {
            ...
            for (String instance : perm.getObjectInstances()) instanceSet.add(instance);
        }
    }
    ...
    public boolean hasPermission(Ace testify) {
        ...
        if (!testify.getObjectClass().equals(MSecurity.WILDCARD_TOKEN)) {
            ...
            if (actions != null && actions.hasPermission(testify)) return true;
            actions = clazzes.get(MSecurity.WILDCARD_TOKEN);
            if (actions != null) return actions.hasPermission(testify);
        } else {
            // distinct wildcard
    ...
    private class Actions {
        ...
        public boolean hasPermission(Ace perm) {
            ...
            if (wildcard != null) if (wildcard.hasPermission(perm)) return true;
            ...
            if (!action.equals(MSecurity.WILDCARD_TOKEN)) {
                ...
                for (Map.Entry<String, Instances> value : wildcards.entrySet())
                    if (action.startsWith(value.getKey()) && value.getValue().hasPermission(perm))
                        return true;
            } else {
                ...
                for (Instances res : entries.values()) {
                    ...
                    for (Map.Entry<String, Instances> value : wildcards.entrySet())
                        if (action.startsWith(value.getKey())
                                && value.getValue().hasPermission(perm)) return true;
                }
            }
            return false;
        }
        ...
        public void put(String action, Instances instances) {
            ...
            if (action.equals(MSecurity.WILDCARD_TOKEN)) {
                if (wildcard == null) wildcard = instances;
                return;
            }
            if (action.endsWith(MSecurity.WILDCARD_TOKEN)
                    && action.length() > MSecurity.WILDCARD_TOKEN.length()) {
                wildcards.put(
                        action.substring(0, action.length() - MSecurity.WILDCARD_TOKEN.length()),
                        instances);
            }
        }
    ...
    private class Instances {
        ...
        public void add(String instance) {
            ...
            if (instance.endsWith(MSecurity.WILDCARD_TOKEN)
                    && instance.length() > MSecurity.WILDCARD_TOKEN.length()) {
                wildcards.add(
                        instance.substring(
                                0, instance.length() - MSecurity.WILDCARD_TOKEN.length()));
            }
        }
        ...
        public boolean hasPermission(Ace perm) {
            ...
            for (String value : wildcards) if (instance.startsWith(value)) return true;
            return false;
        }
