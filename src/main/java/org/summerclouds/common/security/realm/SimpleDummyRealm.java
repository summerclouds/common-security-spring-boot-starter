...
/**
 * This realm simply define two users
 * 1. user
 * 2. admin
 * 
 * @author mikehummel
...
public class SimpleDummyRealm implements UserRealm {
	...
	private synchronized User getUserGuest() {
		...
		guest = new RealmUser(this, "guest", UUID.randomUUID().toString(), true, true, true, true, auth);
		return guest;
	}
...
//			auth.add(new SimpleGrantedAuthority(UUID.randomUUID().toString()));
	...
	private synchronized User getUserAdmin() {
		...
		admin = new RealmUser(this, "admin", encoder.encode(password), true, true, true, true, auth);
